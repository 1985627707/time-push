...
def get_birthday(birthday, year, today):
    global birthday_month, birthday_day, r_mouth, r_day
...
def zimi():
    if (zimi_API!=""):
        ...
        params = urllib.parse.urlencode({'key': zimi_API})
        headers = {'Content-type': 'application/x-www-form-urlencoded'}
        conn.request('POST', '/zimi/index', params, headers)
        res = conn.getresponse()
        data = res.read()
        data = json.loads(data)
        dataContent = data["newslist"][0]["content"]
        dataAnswer=data["newslist"][0]["answer"]
        dataReason = data["newslist"][0]["reason"]
        data=str(dataContent)+";"+str(dataAnswer)+";"+str(dataReason)
        return data
    else:
        return ""
...
def joke():
    if (joke_API!="否"):
        conn = http.client.HTTPSConnection('api.tianapi.com')  #接口域名
        params = urllib.parse.urlencode({'key':joke_API,'num':1})
        headers = {'Content-type':'application/x-www-form-urlencoded'}
        conn.request('POST','/joke/index',params,headers)
        res = conn.getresponse()
        data = res.read()
        data = json.loads(data)
        data =str(data["newslist"][0]["title"])+":"+str(data["newslist"][0]["content"])
        return data
    else:
        return ""
#励志名言
...
#推送信息
def send_message(to_user, access_token, city_name, weather, max_temperature, min_temperature, pipi, lizhi, pop, tips, note_en, note_ch, health_tip, lucky_,joke,qianyu):
...
if __name__ == "__main__":
    ...
    lucky_ = ""#lucky()
    #倩宝语录：
    qianyu="处暑过，秋将至，随雨下绿植入朦胧，似雾似梦"
    # 公众号推送消息
    for user in users:
        send_message(user, accessToken, city, weather, max_temperature, min_temperature, pipi, lizhi,pop,tips, note_en, note_ch, health_tip, lucky_,xiaoh,qianyu)
    import time
    time_duration = 3.5
